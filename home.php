...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($post) {
            ...
            if (!$found) {
                ...
                if ($post['user_id'] !== $currentUser['id']) {
                    createNotification($post['user_id'], 'like_post', $currentUser['id'], $postId, "{$currentUser['name']} le gustó tu publicación");
                }
            }
    ...
    if ($action === 'retweet_post') {
        ...
        if ($post) {
            ...
            if (updatePost($postId, ['retweets' => $retweets])) {
                if ($post['user_id'] !== $currentUser['id']) {
                    createNotification($post['user_id'], 'retweet', $currentUser['id'], $postId, "{$currentUser['name']} retweeted tu publicación");
                }
                $updatedPost = getPostById($postId);
                $response['success'] = true;
    ...
    if ($action === 'create_note') {
        ...
        if (!empty($content)) {
            ...
            $response['message'] = 'La nota está vacía';
        }
        echo json_encode($response);
        exit;
    }
    ...
    if ($action === 'delete_note') {
        ...
        if (deleteNote($noteId, $currentUser['id'])) {
            $response['success'] = true;
        } else {
    ...
    if ($action === 'like_note') {
        ...
        if ($note) {
            ...
            if ($alreadyLiked) {
                $result = unlikeNote($noteId, $currentUser['id']);
            } else {
                $result = likeNote($noteId, $currentUser['id'], $currentUser['name'], $currentUser['avatar']);
            }
            
            if ($result) {
                $updatedNotes = getAllNotesFromBlocks();
                foreach ($updatedNotes as $n) {
                    if ($n['id'] === $noteId) {
                        $response['success'] = true;
                        $response['likes'] = count($n['likes']);
                        $response['liked'] = !$alreadyLiked;
                        break;
                    }
                }
            } else {
                $response['message'] = 'Error al procesar like';
            }
        } else {
    ...
    if ($action === 'mark_all_notifications_read') {
        if (markAllNotificationsAsRead($currentUser['id'])) {
            $response['success'] = true;
        } else {
...
if ($currentView === 'home' && !$userId) {
    ...
    $allPostsData = getPosts($page, 1000, null, null);
...
$myNotes = getMyNotes($currentUser['id']);
$publicNotes = getNotesForUser($currentUser['id']);

$notificationsPage = isset($_GET['page_notif']) ? (int)$_GET['page_notif'] : 1;
$notificationsPerPage = 20;
$allNotifications = getNotifications($currentUser['id'], 1000);
$totalNotifications = count($allNotifications);
$totalNotifPages = ceil($totalNotifications / $notificationsPerPage);
$startNotif = ($notificationsPage - 1) * $notificationsPerPage;
$paginatedNotifications = array_slice($allNotifications, $startNotif, $notificationsPerPage);
$unreadCount = getUnreadNotificationsCount($currentUser['id']);
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
        :root { --bg-primary: #000000; --bg-secondary: #000000; --bg-card: #000000; --bg-hover: #080808; --border-color: #2f3336; --text-primary: #e7e9ea; --text-secondary: #71767b; --accent: #1d9bf0; --like-color: #f91880; --nav-height: 60px; --note-bg: #1a1a2e; }
        body.light { --bg-primary: #ffffff; --bg-secondary: #ffffff; --bg-card: #ffffff; --bg-hover: #f7f7f7; --border-color: #eff3f4; --text-primary: #0f1419; --text-secondary: #536471; --accent: #1d9bf0; --like-color: #f91880; --note-bg: #f0f2f5; }
        ...
        .sidebar-left { width: 275px; position: sticky; top: 0; height: 100vh; padding: 8px 12px; border-right: 1px solid var(--border-color); background-color: var(--bg-primary); overflow-y: auto; }
        .logo { padding: 12px; margin-bottom: 20px; }
        .logo img { height: 35px; width: auto; border: none; padding: 6px; background: red; border-radius: 5px; }
        .nav-menu { display: flex; flex-direction: column; gap: 8px; }
        .nav-item { display: flex; align-items: center; gap: 16px; padding: 12px; border-radius: 30px; cursor: pointer; transition: background 0.2s; color: var(--text-primary); text-decoration: none; font-size: 20px; position: relative; }
        .nav-item i { font-size: 26px; width: 30px; }
        .nav-item:hover, .profile-card:hover { background-color: var(--bg-hover); }
        .nav-item.active { font-weight: 700; }
        .notification-badge { position: absolute; left: 30px; top: 8px; background-color: var(--accent); color: white; font-size: 11px; border-radius: 50%; padding: 2px 6px; min-width: 18px; text-align: center; }
        .profile-card { margin-top: 40px; padding: 12px; display: flex; align-items: center; gap: 12px; cursor: pointer; border-radius: 30px; text-decoration: none; color: var(--text-primary); }
        ...
        .bottom-nav-item { display: flex; flex-direction: column; align-items: center; gap: 4px; text-decoration: none; color: var(--text-secondary); font-size: 12px; transition: color 0.2s; padding: 4px 0; position: relative; }
        ...
        .mobile-notif-badge { position: absolute; top: -5px; right: -10px; background: var(--accent); color: white; font-size: 10px; border-radius: 50%; padding: 2px 5px; min-width: 16px; text-align: center; }
        ...
        .tabs-container { display: flex; border-bottom: 1px solid var(--border-color); background: var(--bg-primary); }
        .tab-btn { flex: 1; padding: 12px; background: none; border: none; color: var(--text-secondary); font-size: 15px; font-weight: 500; cursor: pointer; transition: all 0.2s; }
        .tab-btn.active { color: var(--accent); border-bottom: 2px solid var(--accent); }
        .tab-content { display: none; }
        .tab-content.active { display: block; }
        
        .notifications-header-actions { display: flex; justify-content: space-between; align-items: center; padding: 12px 16px; border-bottom: 1px solid var(--border-color); }
        .mark-all-btn { background: none; border: none; color: var(--accent); cursor: pointer; font-size: 14px; padding: 8px 12px; border-radius: 20px; }
        .mark-all-btn:hover { background: var(--bg-hover); }
        .notification-item-page { display: flex; align-items: center; gap: 12px; padding: 12px 16px; border-bottom: 1px solid var(--border-color); cursor: pointer; transition: background 0.2s; }
        .notification-item-page:hover { background: var(--bg-hover); }
        .notification-item-page.unread { background: rgba(29, 155, 240, 0.1); }
        .notification-avatar { width: 48px; height: 48px; border-radius: 50%; object-fit: cover; }
        .notification-content { flex: 1; }
        .notification-message { font-size: 14px; margin-bottom: 4px; line-height: 1.4; }
        .notification-time { font-size: 11px; color: var(--text-secondary); }
        .notification-delete { background: none; border: none; color: var(--text-secondary); cursor: pointer; padding: 8px; border-radius: 50%; }
        .notification-delete:hover { color: #f4212e; background: var(--bg-hover); }
        .empty-notifications-page { text-align: center; padding: 60px 20px; color: var(--text-secondary); }
        .notif-pagination { display: flex; justify-content: center; gap: 8px; padding: 20px; border-top: 1px solid var(--border-color); }
        ...
        .notes-section { padding: 16px; border-bottom: 1px solid var(--border-color); background: var(--note-bg); margin-bottom: 8px; }
        .create-note-form { display: flex; flex-direction: column; gap: 12px; }
        .note-input { background: var(--bg-hover); border: 1px solid var(--border-color); border-radius: 16px; padding: 12px; color: var(--text-primary); font-size: 15px; resize: none; font-family: inherit; }
        .note-visibility { display: flex; gap: 16px; align-items: center; }
        .note-visibility label { display: flex; align-items: center; gap: 6px; cursor: pointer; font-size: 14px; }
        .note-visibility input { cursor: pointer; }
        .submit-note { background: var(--accent); color: white; border: none; border-radius: 24px; padding: 8px 20px; font-weight: 700; cursor: pointer; align-self: flex-end; }
        .note-card { background: var(--bg-hover); border-radius: 16px; padding: 12px 16px; margin-bottom: 12px; }
        .note-header { display: flex; align-items: center; gap: 12px; margin-bottom: 8px; }
        .note-avatar { width: 36px; height: 36px; border-radius: 50%; object-fit: cover; cursor: pointer; }
        .note-info { flex: 1; }
        .note-author { font-weight: 700; font-size: 14px; cursor: pointer; }
        .note-author:hover { text-decoration: underline; }
        .note-username { font-size: 12px; color: var(--text-secondary); }
        .note-time { font-size: 11px; color: var(--text-secondary); }
        .note-content { font-size: 14px; line-height: 1.4; margin-bottom: 8px; }
        .note-visibility-badge { display: inline-block; font-size: 10px; background: var(--border-color); border-radius: 12px; padding: 2px 8px; margin-bottom: 8px; }
        .note-visibility-badge i { margin-right: 4px; font-size: 10px; }
        .note-actions { display: flex; gap: 16px; margin-top: 8px; }
        .note-action-btn { background: none; border: none; color: var(--text-secondary); font-size: 13px; cursor: pointer; display: flex; align-items: center; gap: 6px; }
        .note-action-btn:hover { color: var(--accent); }
        .note-action-btn.liked { color: var(--like-color); }
        ...
        .user-list-item { display: flex; align-items: center; gap: 12px; padding: 12px; cursor: pointer; }
...
<div class="app-container">
    <div class="sidebar-left">
        <div class="logo"><img src="https://ls.dilivel.com/img/logico.png" alt="Logo"></div>
        <div class="nav-menu">
            <a href="home.php?view=home" class="nav-item <?php echo $currentView === 'home' && !$userId ? 'active' : ''; ?>"><i class="fas fa-home"></i><span>Inicio</span></a>
            <a href="home.php?view=explore" class="nav-item <?php echo $currentView === 'explore' ? 'active' : ''; ?>"><i class="fas fa-hashtag"></i><span>Explorar</span></a>
            <a href="home.php?view=notifications" class="nav-item <?php echo $currentView === 'notifications' ? 'active' : ''; ?>"><i class="fas fa-bell"></i><span>Notificaciones</span><?php if ($unreadCount > 0 && $currentView !== 'notifications'): ?><span class="notification-badge"><?php echo $unreadCount; ?></span><?php endif; ?></a>
            <a href="home.php?view=profile" class="nav-item <?php echo $currentView === 'profile' && !$userId ? 'active' : ''; ?>"><i class="fas fa-user"></i><span>Perfil</span></a>
        </div>
    ...
    <div class="main-feed">
        <div class="feed-header">
            <h2>
                ...
                if ($username && $viewedUser) {
                    echo '@' . escapeHtml($viewedUser['name']);
            ...
            <div style="display: flex; gap: 12px;">
                <?php if ($currentView !== 'notifications'): ?>
                <a href="home.php?view=notifications" class="logout-btn" style="position: relative;">
                    <i class="fas fa-bell"></i>
                    <?php if ($unreadCount > 0): ?>
                    <span class="notification-badge" style="position: absolute; top: -5px; right: -10px;"><?php echo $unreadCount; ?></span>
                    <?php endif; ?>
                </a>
                <?php endif; ?>
        ...
        <?php if ($currentView === 'notifications'): ?>
            <div class="notifications-header-actions">
                <span></span>
                <button class="mark-all-btn" id="markAllReadPageBtn"><i class="fas fa-check-double"></i> Marcar todas como leídas</button>
            </div>
            <div id="notificationsPageFeed">
                <?php if (empty($paginatedNotifications)): ?>
                    <div class="empty-notifications-page">
                        <i class="fas fa-bell-slash" style="font-size: 48px; opacity: 0.5;"></i>
                        <p style="margin-top: 16px;">No hay notificaciones</p>
                    </div>
                <?php else: ?>
                    <?php foreach ($paginatedNotifications as $notif): ?>
                        <div class="notification-item-page <?php echo !$notif['read'] ? 'unread' : ''; ?>" data-notif-id="<?php echo escapeHtml($notif['id']); ?>" data-reference-id="<?php echo escapeHtml($notif['reference_id']); ?>" data-type="<?php echo escapeHtml($notif['type']); ?>" data-from-user="<?php echo escapeHtml($notif['from_user_id']); ?>">
                            <img class="notification-avatar" src="<?php echo escapeHtml($notif['from_user_avatar']); ?>" alt="avatar">
                            <div class="notification-content">
                                <div class="notification-message">
                                    <strong><?php echo escapeHtml($notif['from_user_name']); ?></strong> <?php echo escapeHtml($notif['message']); ?>
                                </div>
                                <div class="notification-time"><?php echo formatTimeAgo($notif['created_at']); ?></div>
                            </div>
                            <button class="notification-delete" onclick="event.stopPropagation(); deleteNotification('<?php echo escapeHtml($notif['id']); ?>')">
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    <?php endforeach; ?>
                    
                    <?php if ($totalNotifPages > 1): ?>
                    <div class="notif-pagination">
                        <?php for ($i = 1; $i <= $totalNotifPages; $i++): ?>
                            <button class="page-btn <?php echo $i == $notificationsPage ? 'active' : ''; ?>" data-page-notif="<?php echo $i; ?>"><?php echo $i; ?></button>
                        <?php endfor; ?>
                    </div>
                    <?php endif; ?>
                <?php endif; ?>
            </div>
        <?php elseif ($username && $viewedUser): ?>
            <div class="profile-card-large">
                <div style="border-bottom: solid 0px transparent;">
            ...
            <div id="notesTab" class="tab-content">
                <div class="notes-section">
                    <div class="create-note-form">
                        <textarea class="note-input" id="noteContent" rows="3" placeholder="Escribe una nota... (máximo 280 caracteres)" maxlength="280"></textarea>
                        <div class="note-visibility">
                            <label>
                                <input type="radio" name="visibility" value="public" checked>Pública
                            </label>
                            <label>
                                <input type="radio" name="visibility" value="friends">Solo amigos
                            </label>
                        </div>
                        <button class="submit-note" id="submitNoteBtn">Publicar Nota</button>
                    </div>
                </div>
                <div id="notesFeed">
                    <div class="loading">Cargando notas...</div>
        ...
        <?php elseif ($currentView === 'explore'): ?>
            <div class="posts-feed" id="postsFeed">
                <div class="loading">Cargando publicaciones...</div>
            </div>
...
<div class="bottom-nav">
    <div class="bottom-nav-items">
        ...
        <a href="home.php?view=notifications" class="bottom-nav-item <?php echo $currentView === 'notifications' ? 'active' : ''; ?>">
            <i class="fas fa-bell"></i>
            <?php if ($unreadCount > 0 && $currentView !== 'notifications'): ?>
            <span class="mobile-notif-badge"><?php echo $unreadCount; ?></span>
            <?php endif; ?>
        </a>
...
const currentUserName = '<?php echo escapeHtml($currentUser['name']); ?>';
const currentUserAvatar = '<?php echo escapeHtml($currentUser['avatar']); ?>';
...
document.querySelectorAll('.notification-item-page').forEach(item => {
    item.addEventListener('click', function(e) {
        ...
        const fromUserId = this.dataset.fromUser;
        
        markNotificationRead(notifId);
        
        if (type === 'like_post' || type === 'retweet' || type === 'comment') {
            if (referenceId) window.location.href = `comment.php?post_id=${referenceId}`;
        } else if (type === 'follow') {
            if (fromUserId) window.location.href = `home.php?user=${fromUserId}`;
        }
    });
});
...
async function markNotificationRead(notifId) {
    ...
    try {
        await fetch('home.php', { method: 'POST', body: formData });
        const item = document.querySelector(`.notification-item-page[data-notif-id="${notifId}"]`);
        if (item) {
...
document.getElementById('markAllReadPageBtn')?.addEventListener('click', markAllNotificationsRead);

document.querySelectorAll('.page-btn[data-page-notif]').forEach(btn => {
    btn.addEventListener('click', function() {
        const page = this.dataset.pageNotif;
        if (page) {
            const url = new URL(window.location.href);
            url.searchParams.set('page_notif', page);
...
async function loadNotes() {
    const feedContainer = document.getElementById('notesFeed');
    if (!feedContainer) return;
    ...
    try {
        ...
        if (data.success) {
            ...
            feedContainer.innerHTML = '<div class="empty-feed"><p>Error al cargar notas</p></div>';
        }
    } catch (error) {
...
function renderNotes(notes) {
    ...
    if (!notes || notes.length === 0) {
        feedContainer.innerHTML = '<div class="empty-feed"><i class="fas fa-sticky-note" style="font-size: 48px; opacity: 0.5;"></i><p style="margin-top: 16px;">No hay notas.</p></div>';
        return;
    }
    
    let html = '';
    for (const note of notes) {
        const isLiked = note.likes && note.likes.some(like => like.user_id === currentUserId);
        const likeCount = note.likes ? note.likes.length : 0;
        const visibilityText = note.visibility === 'public' ? 'Pública' : 'Solo amigos';
        const visibilityIcon = note.visibility === 'public' ? 'fa-globe' : 'fa-user-friends';
        
        html += `
            <div class="note-card" data-note-id="${escapeHtml(note.id)}">
                <div class="note-header">
                    <img class="note-avatar" src="${escapeHtml(note.author_avatar)}" alt="avatar" onclick="location.href='home.php?user=${encodeURIComponent(note.author_username)}'">
                    <div class="note-info">
                        <div class="note-author" onclick="location.href='home.php?user=${encodeURIComponent(note.author_username)}'">${escapeHtml(note.author_name)}</div>
                        <div class="note-username">@${escapeHtml(note.author_username)} · ${formatTimeAgo(note.created_at)}</div>
                    </div>
                    ${note.user_id === currentUserId ? `
                    <button class="menu-btn" onclick="deleteNote('${escapeHtml(note.id)}')" style="background: none; border: none; color: var(--text-secondary); cursor: pointer;">
                        <i class="fas fa-trash"></i>
                    </button>
                    ` : ''}
                </div>
                <div class="note-visibility-badge">
                    <i class="fas ${visibilityIcon}"></i> ${visibilityText}
                </div>
                <div class="note-content">${linkifyText(note.content)}</div>
                <div class="note-actions">
                    <button class="note-action-btn like-note-btn ${isLiked ? 'liked' : ''}" onclick="toggleLikeNote('${escapeHtml(note.id)}')">
                        <i class="${isLiked ? 'fas fa-heart' : 'far fa-heart'}"></i>
                        <span class="like-count">${likeCount}</span>
                    </button>
                </div>
...
async function toggleLikeNote(noteId) {
    const formData = new FormData();
    formData.append('action', 'like_note');
    formData.append('note_id', noteId);
...
async function toggleFollow(userId) {
    ...
    formData.append('follow', !isCurrentlyFollowing);
...
if (document.getElementById('notesFeed') && (currentView === 'profile' || currentView === 'home')) {
    loadNotes();
}
</script>
</body>
